function Y_hat = random_forest(original_train_data, test_data, categorical_column_label, varargin)
%% varargin
%%  T: number of trees
%%  sub_attr_percent: percentage of sub attrbute set


T = 20;
sub_attr_percent = 0.2;
crtiterion_indicator = 0;
prune_thredshold = 0; % no prune
...
elseif length(varargin{1,1}) == 2
    T = varargin{1,1}{1};
    sub_attr_percent = varargin{1,1}{2};
elseif length(varargin{1,1}) > 2
...
n_train = size(original_train_data,1);
d = size(original_train_data,2)-1;
original_train_X = original_train_data(:,1:d);
original_train_Y = original_train_data(:,d+1);
Y_hat_all = zeros(n_test,T);
Y_hat = zeros(n_test,1);

for iter=1:T
    %% sample data
    train_data = datasample(original_train_data, n_train);
    ...
    while n_pure_leaves < n_leaves
        new_impurity_attributes = ones(1,d);
        attributes_splitor = {};
        ...
        % choose subset attribute
        sub_attribute = randsample(1:d, ceil(d*sub_attr_percent));
        for i=1:d
            if any(sub_attribute == i)
