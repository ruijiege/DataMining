function demo1(varargin)

train_data = getfield(load('dataset3_train.mat'),'data_matrix');
test_data = getfield(load('dataset3_test.mat'),'data_matrix');
categorical_label = getfield(load('categorical_indicator3.mat'),'categorical_column_label');

Y = test_data(:,size(test_data,2));
Y_hat = nearest_neighbor(train_data, test_data, categorical_label, varargin);
